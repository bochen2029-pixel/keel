...
- **L1 substrate resolver — (c1) probe/resolve landed** (`kernel::lifecycle`): `keel "…"` now **discovers** the local server by probing the candidate ports (LM Studio :1234 · Ollama :11434 · llama-server :8080) and routes to the first live one — `SUBSTRATE_UNRESOLVED` if none. Dependency-free TCP-connect probe (kernel stays tiny). **Live**: CLI prints `local substrate → http://127.0.0.1:8080`. 4 self-contained lifecycle tests. **(c2) launch + supervise next.**
...
- ~~**a (deepseek)** · **c1 (probe/resolve)**~~ ✓. **Next: (c2)** `lifecycle` launch + supervise — spawn llama-server when none is up (keel.lock path → `/health` poll → 3-strike restart, one long-lived subprocess) → **(b)** `serve_openai` (consume KEEL over protocol). Plus kernel `engine` · `lock` · `store::sqlite`. Then Stage 1 (router · amplify · perception).
