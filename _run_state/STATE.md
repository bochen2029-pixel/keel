...
- ~~**path to a real tier**: `local_llama` adapter~~ ✓ live against b9627. **Next focus = wire it into a runnable binary** (app/CLI, L5): keel.lock → registry(+local_llama) → chain(audit·privacy·cost) → real tier, with a **file-backed `AuditSink`** (the ledger; I1/I2 to disk). That is the Stage-0 capstone — "resolves the substrate, talks to a tier, logs every call." Then deepseek adapter + kernel engine/lifecycle/lock.
