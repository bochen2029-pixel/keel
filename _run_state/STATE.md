...
- ~~**a (deepseek)** · **c (resolver: probe + launch/supervise)**~~ ✓ — KEEL self-sufficient (cold-starts its own substrate). **Next: (b)** `serve_openai` — expose KEEL over protocol (OpenAI-compatible HTTP server) so the fleet (DAVE/TERMINAL/…) consumes it. Plus kernel `engine` · `lock` · `store::sqlite` · auto-restart supervision loop (the `is_alive`/`kill` primitives exist; the loop lands with a long-lived host). Then Stage 1 (router · amplify · perception).
