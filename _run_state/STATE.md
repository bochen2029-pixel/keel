...
- **L2 cheap-API tier landed** (`keel-adapters::deepseek`): DeepSeek over HTTPS (`/chat/completions` no `/v1`; `thinking`+`reasoning_effort`), reusing the shared `openai` mapping (the factor-out pays off). **Live-validated** on the real API — correct answer, `reasoning_content` parsed, **real cost** computed; the CLI is now `--tier`-aware (local | cheap-API) with the **I3 egress mask on for cloud tiers** (local stays sovereign-safe). The ledger records both tiers' real costs (local $0 · cheap-API $0.000126). Key in `DEEPSEEK_API_KEY` (env), never a file. **36 unit tests + 2 live.**
...
- ~~**real tier** · **runnable binary** · **cheap-API tier (deepseek)**~~ ✓ — two-tier economy runs end-to-end with real-cost I4 + the I3 egress mask. **Remaining Stage-0 (order a→c→b; a done): (c)** kernel `lifecycle` + substrate-resolver (discover/launch/supervise llama-server) → **(b)** `serve_openai` (consume KEEL over protocol). Plus kernel `engine` · `lock` · `store::sqlite`. Then Stage 1 (router · amplify · perception).
