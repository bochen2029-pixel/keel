...
- **The next slice:** the **serve→Director bridge** — `serve_openai` gains `critical`/`golden_ref` + a constrained-decode path, then the Director dogfood closes the I5 **accept direction** in-binary (a schema-valid Directive PASSES via a ref) and lands the first external consumer. Then **perception** (eyes + ears), then **`svc::memory`** (design most skeptically against `docs/proposals/perpetual-memory.md`, the canon's flagged **highest-risk seam**). *(Done 2026-06-14: engine + freeze-gate + constrained-decode + I5 teeth (dispatch · #3 un-neutered · ASCII alarm) + **`metrics`** — the off-loop reader (`keel metrics`). So far the gate is proven REJECTING; the accept direction in-binary is what the Director bridge closes. See §4 / §5.1.)*
