...
- **L1 `keel-kernel`** — `manifest · context · registry · chain · lifecycle`(+probe/launch resolver). **`engine` and `lock` modules do NOT exist yet** (declared "next").
...
- **L5 `keel`** — the `keel` CLI **and** `keel-serve` (axum OpenAI server), sharing one `assemble()`; plus the self-driving `keel::Engine`.
- **Goldens:** 21 cases / 6 families (router · model_tier · oracle · perception · recall · privacy), **RATIFIED + FROZEN** (`golden.json` `_meta`, 2026-06-13). Seal `63d5ba7c…` (Marrow-Python provenance; freeze-gate `#[ignore]`-dormant — see 4.4).
...
## 4.2 · Invariant scorecard (honest — real vs aspirational)
- **I1 audit** — ✅ **enforced** in-chain → JSONL ledger (fires even on a blocked call). *Gap:* redactions are not themselves audited (canon §5.1 wants them to be).
- **I2 durable** — 🟡 **partial.** `Spine` impl exists (`store::sqlite`) but **the loop never checkpoints**, and **no `Memory` impl exists at all.** In-turn, only the audit ledger persists.
- **I3 sovereign** — 🟡 **partial.** Gate (router force-local) + mask (per-tier egress) both present; but **rung-1 operator markers are an empty list** (`Redactor::new(vec![])`), rung-2 is narrow (no phone/URL), redactions are unaudited, and rung-3 (the model) is deferred to Stage 2.
- **I4 cost** — 🟡 **partial.** The hard-stop gate is enforced, **but mid-run cost is never folded into `Context`** — so the gate only ever sees the *initial* budget. Latent today (turns are single-shot) but it bites the moment a real multi-call loop exists.
- **I5 externalized** — 🟡 **partial — and this is THE gap.** The verifier is golden-green at L4, **but `Engine::run` never calls it; `Step.oracle_failures` stays 0, so the escalation ladder can never fire.** "Ground every critical output in a non-model assertion" is **aspirational in the running binary.** Wiring this is the next slice.
- **Reversibility** — ✅ policy-enforced; **the system remains SUPERVISED** (no codified unattended-autonomy grant — see 4.4).

**Plain truth:** KEEL today is an excellent sovereign **router + substrate**. The parts that make it *KEEL rather than a competent two-tier wrapper* — externalized correctness *in the loop* (I5), in-turn memory (I2), and the flywheel — are exactly the parts still ahead. The first is one slice away.
...
2. **`CLAUDE.md` build-state — STALE, do not trust for state.** It still reads "Next: Stage 0, nothing above L0 exists yet" and "goldens PROPOSED." Both are false. **Trust `STATE.md` + `git` for state; use `CLAUDE.md` only for the rules/disciplines** (which remain correct). Correcting CLAUDE.md's build-state + first-cell line is an operator-governed fix on the roadmap.
3. **"KEEL self-contained vs the Marrow bench" — an open reconciliation.** A directive holds that KEEL is self-contained / not Marrow-dependent; yet `CLAUDE.md` commands diffing against `C:\loom\marrow-l1`. **The working resolution:** the Marrow bench is a *reference diff-oracle* you read for behavior, **not** a runtime/build dependency — KEEL ships nothing of Marrow's and the genesis transcript is preserved in `_memories\`. Whether to keep or cut the bench reference is an operator decision; until ruled, treat Marrow as read-only reference, never a dependency.
...
5. **The freeze-gate seal — operator-only re-stamp pending** (see 4.4). Do not self-stamp.

## 4.4 · Known gaps & debt (the to-fix register)
- **HIGH — I5 not in the loop.** `Engine::run` doesn't call `verifier.verify`; `oracle_failures` never feeds back. *(Fixed by the `kernel::engine` slice — Part 5.)*
- **HIGH — I4 cost not accumulated mid-run.** The fold of `result.cost → Context.cost` is the engine's job and the engine doesn't do it. *(Same slice fixes it.)*
- **MEDIUM — L5→L1 engine debt.** The loop lives in `keel::Engine` (L5); the canon wants `kernel::engine` (L1) over injected `&dyn Router/Oracle/Memory/Spine/TraceSink`. *(Same slice pays it.)*
- **MEDIUM — golden freeze-gate dormant.** `keel-contracts/tests/golden_freeze.rs` is `#[ignore]`'d because the stored seal `63d5ba7c…` is Marrow-Python-derived (float `1e-06` vs Rust `1e-6`); the **content is git-verified unchanged** (`db4377b3…` KEEL-native). Resolution = the operator runs `cargo run -p keel-contracts --example golden_freeze -- --update`, then un-ignore. **You must NOT self-stamp** — re-authoring frozen ground truth is structurally the operator's.
- **MEDIUM — doc drift** (CLAUDE.md build-state + the Director line in STATE.md). Operator-governed.
- **LOW — config hard-coded** in `keel/src/lib.rs` (substrate paths, `max_tokens=2048`) instead of read from `keel.lock` (declared-but-not-driving).
...
- **LATER — no in-turn memory:** no `Memory` impl; the loop doesn't hydrate context or checkpoint. (Stage 2.)
