...
**The three consumption destinies + scale-invariance (the operator's framing, captured 2026-06-14).** KEEL exists because its operator kept re-building the *same* AI substrate by hand across his own projects (the Qwen3.5-9B local vision-LLM, Whisper, privacy filtering, cheap↔frontier routing) — so KEEL builds that common core **once**, maximally reusable, and never rewrites it (you cannot embed an OpenCode/Claude-Code *agent* inside a game; KEEL is a *substrate you embed*). One frozen core serves three destinies, differing only by which modules are toggled: **(a)** an embeddable, reusable AI bundle inside his own apps and games — *the .NET of his AI apps*; **(b)** his own sovereign personal harness/assistant (OpenClaw-class, but local-first and grounded by a non-model oracle); **(c)** aspirationally, the orchestration kernel of an intelligence that runs an entire organization. The load-bearing property is **scale-invariance and case-agnosticism**: the simple invariant loop (§8) + the ten contracts (§7) reduce to the minimal case for a game's AI module and extend to the org-scale case **purely by which modules are slotted, toggled, or swapped — never by editing the loop or a contract.** (A native C/C++ rewrite is a contemplated *future* once KEEL is fully implemented; ADR #5's language-neutral golden cases make it a re-pass-the-same-goldens port — the language is the most reversible decision, the contracts are the longevity asset.)
...
| `store::sqlite` | L2 | the index behind a `Store` seam (+ `sqlite-vec`); the ledger is files; serves derived **metric rollups** (`escalation_rate`/`rework_rate`/by-tier/cost) as an off-loop **reader** | 6/7 |
...
- **Stage 2 — Correctness & memory.** the agent-frozen golden registry + freeze check; `svc.verifier` (oracle registry + joint-wrong); ringed `svc.memory` (Tape ledger, consolidation-as-a-Step, narrative/factual registers); `metrics` (`escalation_rate`, `rework_rate`) — a **reader** over the I2 index (off-loop, read-only; **not middleware** — a future real-time metric-driven gate, e.g. tripping on rising `rework_rate` mid-run, would be). **Outcome:** critical outputs carry a non-model assertion; the substrate measures itself. **Falsifier:** if `rework_rate` can't be driven < 10% with oracles on, the oracle taxonomy needs rework before more autonomy.
